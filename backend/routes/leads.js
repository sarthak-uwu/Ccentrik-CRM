...
// GET /api/leads/:id — field users can only access records assigned to them
router.get("/:id", authenticate, async (req, res) => {
  ...
  // Field users cannot access records not assigned to them
  if (role !== "owner" && role !== "sales_head" && role !== "sales_manager") {
    if (data.assigned_to !== profileId && data.created_by !== profileId) {
...
// Shared handler for PUT and PATCH — admins can edit any record;
// field users (employee / inside_sales) can only edit their own pipeline records.
async function handleLeadUpdate(req, res) {
  ...
  if (!isAdmin) {
    const { data: existing, error: fetchErr } = await supabase
      .from("leads")
      .select("assigned_to, created_by, stage")
      .eq("id", req.params.id)
      .single();
    if (fetchErr || !existing) return res.status(404).json({ error: "Record not found" });
    if (existing.stage !== "pipeline") return res.status(403).json({ error: "You can only edit your own pipeline records" });
    if (existing.assigned_to !== profileId && existing.created_by !== profileId) {
      return res.status(403).json({ error: "Access denied — you can only edit records assigned to you" });
    }
    // Field users cannot reassign ownership
  ...
  if (oldLead) logLeadChanges(req.params.id, oldLead, safeBody, profileId).catch(() => {});
  res.json(data);
}

// PUT /api/leads/:id
router.put("/:id", authenticate, handleLeadUpdate);

// PATCH /api/leads/:id
router.patch("/:id", authenticate, handleLeadUpdate);

// DELETE /api/leads/:id — admins can delete any; field users can delete their own pipeline records only
router.delete("/:id", authenticate, async (req, res) => {
  const { role, id: profileId } = req.profile;
  const isAdmin = ["owner", "sales_head"].includes(role);

  // Fetch lead before deletion (needed for both ownership check and audit trail)
  const { data: lead } = await supabase
    .from("leads").select("contact_name,company_name,lead_code,assigned_to,created_by,stage").eq("id", req.params.id).single();

  if (!lead) return res.status(404).json({ error: "Record not found" });

  if (!isAdmin) {
    if (lead.stage !== "pipeline") return res.status(403).json({ error: "You can only delete your own pipeline records" });
    if (lead.assigned_to !== profileId && lead.created_by !== profileId) {
      return res.status(403).json({ error: "Access denied — you can only delete records assigned to you" });
    }
  }
...
router.post("/:id/convert-to-lead", authenticate, async (req, res) => {
  try {
    const { data: lead, error: fetchErr } = await supabase
      .from("leads")
      .select("id, stage, company_name, contact_name, email, phone, other_notes, contacts")
      .eq("id", req.params.id)
      .single();
    ...
    // contacts JSONB array (LinkedIn, WhatsApp, additional methods)
    ...
    // Phone: direct field, notes.phone, notes.whatsapp, contacts array, people_contacts
    let hasPhone = !!(lead.phone?.trim() || notes.phone?.trim() || notes.whatsapp?.trim());
    if (!hasPhone) hasPhone = contactsArr.some((c) => (c.type === "phone" || c.type === "whatsapp") && c.value?.trim());
    if (!hasPhone && Array.isArray(notes.people_contacts)) {
      hasPhone = notes.people_contacts.some((p) => p?.phone?.trim());
    }

    // Email fallback: contacts array, people_contacts
    let hasEmailFull = hasEmail;
    if (!hasEmailFull) hasEmailFull = contactsArr.some((c) => c.type === "email" && c.value?.trim());
