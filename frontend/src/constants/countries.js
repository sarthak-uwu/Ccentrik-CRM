// Single source of truth for all country data across the CRM.
// ISO 3166-1 — sorted A→Z by name — code + name + dial.
export const COUNTRIES = [
  ...
  { code: "VC", name: "Saint Vincent and Grenadines",  dial: "+1"    },
...
// Utility: resolve a stored value (code or name) to the matching country object
export function findCountry(value) {
  if (!value) return null;
  const v = value.trim();
  return (
    COUNTRIES.find((c) => c.code === v) ||
...
// Utility: resolve stored value to display name, falls back to raw value
export function countryName(value) {
  if (!value) return "";
  return findCountry(value)?.name || value;
}

// Utility: ISO 3166-1 alpha-2 code → flag emoji (kept for backwards compat)
...
// Utility: ISO 3166-1 alpha-2 code → flag image URL (reliable on all OS including Windows)
