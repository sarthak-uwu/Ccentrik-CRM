          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
            ...
            {contact.linkedin && (
              <a href={contact.linkedin.startsWith("http") ? contact.linkedin : `https://${contact.linkedin}`} target="_blank" rel="noopener noreferrer"
                style={{ fontSize: 12, color: "#0A66C2", textDecoration: "none", display: "flex", alignItems: "center", gap: 5 }}>
                <Link2 size={11} strokeWidth={1.8} />LinkedIn Profile
              </a>
            )}
...
function AddPeopleContactInlineForm({ onAdd, onCancel }) {
  const [form, setForm] = useState({ name: "", designation: "", email: "", phone: "", dial: "+91", country: "IN", state: "", city: "", linkedin: "", notes: "" });
  ...
  const handleAdd = () => {
    ...
    setForm({ name: "", designation: "", email: "", phone: "", dial: "+91", country: "IN", state: "", city: "", linkedin: "", notes: "" });
  };
    ...
    <div style={{ padding: "16px", background: "var(--surface-2)", borderRadius: 12, border: "1.5px solid var(--accent)", marginBottom: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
        ...
        <div style={{ gridColumn: "1 / -1", position: "relative" }}>
          <label style={CONTACT_LABEL_STYLE}>LinkedIn Profile URL</label>
          <input className="crm-input" value={form.linkedin} onChange={(e) => set("linkedin", e.target.value)} placeholder="https://linkedin.com/in/username" style={{ height: 36, paddingRight: form.linkedin ? 36 : undefined }} />
          {form.linkedin && (
            <a href={form.linkedin.startsWith("http") ? form.linkedin : `https://${form.linkedin}`} target="_blank" rel="noopener noreferrer" title="Open LinkedIn profile"
              style={{ position: "absolute", right: 10, top: "calc(50% + 9px)", transform: "translateY(-50%)", color: "#0A66C2", display: "flex", alignItems: "center" }}>
...
// ── Activity highlight pulse style (injected once) ───────────────────────────
if (typeof document !== "undefined" && !document.getElementById("act-highlight-style")) {
  const s = document.createElement("style");
  s.id = "act-highlight-style";
  s.textContent = `
    @keyframes actPulse {
      0%   { box-shadow: 0 0 0 0 rgba(245,158,11,0.55); background: rgba(245,158,11,0.18); }
      50%  { box-shadow: 0 0 0 8px rgba(245,158,11,0); background: rgba(245,158,11,0.10); }
      100% { box-shadow: 0 0 0 0 rgba(245,158,11,0); background: transparent; }
    }
    .act-highlight { animation: actPulse 1.4s ease 3; border-radius: 10px; }
  `;
  document.head.appendChild(s);
}

// ── Main Panel ────────────────────────────────────────────────────────────────
export default function PipelineDetailPanel({ entry, onClose, onEdit, onConvert, pipelineLocked = false, highlightActivityId = null, onHighlightConsumed }) {
  ...
  const [activeTab, setActiveTab] = useState(() => highlightActivityId ? "timeline" : "details");
  ...
  // Scroll to + pulse-highlight the specific activity from the dashboard alert
  useEffect(() => {
    if (!highlightActivityId || actLoading) return;
    const timer = setTimeout(() => {
      if (highlightRef.current) {
        highlightRef.current.scrollIntoView({ behavior: "smooth", block: "center" });
        highlightRef.current.classList.add("act-highlight");
        onHighlightConsumed?.();
      }
    }, 400);
    return () => clearTimeout(timer);
  }, [highlightActivityId, actLoading, onHighlightConsumed]);
        ...
        <div style={{ padding: "20px 24px 0", borderBottom: "1px solid var(--border)", background: "var(--surface)", flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 14 }}>
            ...
            <div style={{ display: "flex", gap: 8, flexShrink: 0, alignItems: "center" }}>
              ...
              {onEdit && canEdit && (isSalesHead || !entry.is_locked) && (
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px 32px" }}>
          ...
          {activeTab === "details" && (
            <div>
              ...
              {!infoMasked && (() => {
                const poc = peopleContacts.find((p) => p.is_primary) || peopleContacts[0];
                const linkedinUrl = poc?.linkedin || extra.linkedin_url;
                return linkedinUrl ? <InfoRow icon={Link2} label="LinkedIn" value={linkedinUrl} isLink /> : null;
              })()}
              ...
              {extra.source && <InfoRow icon={Tag} label="Source" value={extra.source} />}
              ...
              {actLoading ? (
                ...
                <div style={{ display: "flex", flexDirection: "column" }}>
                  {activities.map((act, i) => {
                    ...
                    return (
                      <div key={act.id} ref={isHighlighted ? highlightRef : null} style={{ padding: isHighlighted ? "6px 8px" : undefined }}>
                        <ActivityItem activity={act} isLast={i === activities.length - 1} />
                      </div>
                    );
                  })}
                </div>
