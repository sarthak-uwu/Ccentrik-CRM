...
import {
  ...
  Lock, LockOpen, Users, Star, Trash2, MapPin,
} from "lucide-react";
...
function InfoRow({ icon: Icon, label, value, isLink, isEmail }) {
...
function LeadPeopleContactCard({ contact, onSetPrimary, onDelete, isPending, canEdit }) {
  return (
...
/* ─── Main Panel ──────────────────────────────────────────────────────────── */
export default function LeadDetailPanel({ lead, onClose, onEdit, onConvert, onRevertPipeline, highlightActivityId = null, onHighlightConsumed }) {
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                ...
                {lead.source && (
                  <span style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "4px 9px", borderRadius: 99, background: "var(--surface-2)", border: "1px solid var(--border)", fontSize: 12, fontWeight: 600, color: "var(--text-muted)" }}>
                    <Tag size={10} strokeWidth={2} /> {lead.source}
                  </span>
                )}
              </div>
            </div>
            ...
            <div style={{ display: "flex", gap: 8, flexShrink: 0, alignItems: "center" }}>
              ...
              {isSalesHead && (
                <motion.button
                  onClick={async (e) => {
                    e.stopPropagation();
                    if (onRevertPipeline) { onRevertPipeline(lead); return; }
                    try {
                      const { error } = await supabase.from("leads").update({ stage: "pipeline", pipeline_stage: "new_prospect", updated_at: new Date().toISOString() }).eq("id", lead.id);
                  ...
                  style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 13px", borderRadius: 10, background: "rgba(99,102,241,0.1)", border: "1px solid rgba(99,102,241,0.28)", color: "#6366F1", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px 32px" }}>
          ...
          {activeTab === "details" && (
            <div>
              ...
              <InfoRow icon={Mail}      label="Email"           value={extra.email || lead.email} isEmail />
              <InfoRow icon={Mail}      label="Alternate Email" value={extra.alternate_email} isEmail />
                ...
                <InfoRow icon={Globe} label="Country"
                  value={
                    <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                      <FlagImg code={extra.country} size={16} />
                      {countryName(extra.country) || extra.country}
                    </span>
                  }
                />
              )}
