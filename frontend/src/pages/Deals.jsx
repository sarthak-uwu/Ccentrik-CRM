...
import { COUNTRIES, findCountry, countryName } from "../constants/countries";
...
import {
  ...
  Filter, ArrowUpDown, Globe, ExternalLink, ChevronLeft, ChevronRight, ChevronDown,
} from "lucide-react";
    ...
    <div ref={ref} style={{ position: "relative", display: "inline-block" }} onClick={(e) => e.stopPropagation()}>
      ...
      {open && (
        <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, zIndex: 400, background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 10, boxShadow: "0 8px 28px rgba(0,0,0,0.14)", padding: "4px 0", minWidth: 175, maxHeight: 240, overflowY: "auto" }} className="custom-scroll">
          <div style={{ padding: "3px 12px 5px", fontSize: 9.5, fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.07em" }}>Deal Source</div>
          {sources.map((s) => {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      try {
        ...
        if (backendErr.message && !backendErr.message.includes("fetch")) throw backendErr;
        // Backend unreachable — fall back to direct Supabase
        if (!deal.lead_id) throw new Error("No linked lead — cannot revert");
        const targetStage = isToLead ? "new" : "pipeline";
        ...
        // Detach activities so FK won't block deletion
...
export default function Deals() {
  ...
  const { data: dealsData, isPending: dealsIsPending, isError } = useQuery({
    queryKey: ["deals", search, viewFilter, profile?.id],
    queryFn: () => dealsService.getAll({ search, assignedTo: assignedToParam, profileId: profile?.id, role: profile?.role }),
    enabled: !!profile?.id,
    retry: 1,
  });
  const isLoading = dealsIsPending;
  ...
  const inlineUpdateMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["deals"] }); toast.success("Updated"); },
    onError: (e) => toast.error(e.message),
  });
  ...
  const totalDealsPages = Math.max(1, Math.ceil(deals.length / PAGE_SIZE));
  const safeDealsPage   = Math.min(dealsPage, totalDealsPages);
  const paginatedDeals  = deals.slice((safeDealsPage - 1) * PAGE_SIZE, safeDealsPage * PAGE_SIZE);
  useEffect(() => { setDealsPage(1); }, [search, filterStage, filterCountry, filterIndustry, filterSource, filterAssigned, filterDateFrom, filterDateTo, filterLeadId, filterValueMin, filterValueMax]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <div style={{ padding: "10px 20px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 8, background: "var(--surface)", flexWrap: "wrap" }}>
        ...
        {/* Pagination — before date filter, only in table view */}
        {view === "list" && (
          <Pagination
            compact
            currentPage={safeDealsPage}
                  ...
                  <table className="crm-table">
                    <thead>
                      <tr>
                        ...
                        {[
                          { key: "deal_id",  label: "ID",         required: true, filterKey: null },
                    ...
                    <tbody>
                      {deals.length === 0 ? (
                        ...
                        const rawCountryCode = d.linked_lead?.country || extra.country || "";
                        const countryObj    = findCountry(rawCountryCode);
                        const countryLabel  = countryObj?.name || countryName(rawCountryCode) || rawCountryCode || "—";
                          ...
                          <tr key={d.id} onClick={() => setSelectedDeal(d)} style={{ cursor: "pointer", background: isChecked ? "rgba(99,102,241,0.04)" : isUnassigned ? "rgba(239,68,68,0.03)" : undefined }}>
                              ...
                              <td>
                                {rawCountryCode ? (
                                  <span style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 12.5, color: "var(--text-2)" }}>
                                    <FlagImg code={countryObj?.code || rawCountryCode} size={18} />
                                    {countryLabel}
                                  </span>
                                ) : <span style={{ color: "var(--text-muted)", fontSize: 12 }}>—</span>}
                              </td>
                            ...
                            {isVisible("source") && (
                              <td onClick={(e) => e.stopPropagation()}>
                                <InlineSourceSelect
                                  value={extra.source || null}
                                  sources={DEAL_SOURCES}
                                  disabled={d.is_locked && !isSalesHead}
                                  onSave={(v) => {
                                    const newNotes = JSON.stringify({ ...extra, source: v });
                                    inlineUpdateMutation.mutate({ id: d.id, payload: { notes: newNotes } });
                                  }}
                                />
                              </td>
                            )}
                            ...
                            {isVisible("linkedin") && (
                              <td>
                                {(() => {
                                  const liUrl = extra.linkedin || extra.company_linkedin || d.linked_lead?.company_linkedin;
                            ...
                            <td onClick={(e) => e.stopPropagation()}>
                              <InlineStageSelect
                                value={d.stage}
                                stages={FUNNEL_STATUSES.filter((s) => s.key !== "won" && s.key !== "lost")}
                                disabled={d.is_locked && !isSalesHead}
                                onSave={(v) => inlineUpdateMutation.mutate({ id: d.id, payload: { stage: v } })}
                              />
                            </td>
                            {isVisible("value")    && (
                              <td>
