...
import {
  ...
  ArrowUpDown, Brain, UserPlus, Filter, CheckCircle2, ChevronLeft, ChevronRight,
} from "lucide-react";
...
// Returns true when a lead has at least one usable contact method (email OR phone).
// Mirrors the backend validation logic so the UI and API stay in sync.
function leadHasContact(lead) {
  ...
  if (contacts.some((c) => (c.type === "email" || c.type === "phone" || c.type === "whatsapp") && c.value?.trim())) return true;
  const people = Array.isArray(extra.people_contacts) ? extra.people_contacts : [];
  return people.some((p) => p?.email?.trim() || p?.phone?.trim());
}
...
// Map DB keys → human labels (includes legacy keys for old rows)
const SOURCE_LABELS = {
  ...
  // legacy
  call: "Cold Call", email: "Email", social: "Social Media",
...
// ─── Revert Lead → Pipeline modal ────────────────────────────────────────────
function RevertToPipelineModal({ lead, onClose }) {
  const { profile } = useAuth();
  const qc = useQueryClient();
  const [saving, setSaving] = useState(false);
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const { error } = await supabase
        .from("leads")
        .update({ stage: "pipeline", pipeline_stage: "new_prospect", updated_at: now })
        .eq("id", lead.id);
      ...
      supabase.from("activities").insert({
        type:        "note",
        title:       `Reverted to Pipeline: ${lead.company_name || lead.contact_name}`,
        description: `Lead moved back to Pipeline by ${profile?.full_name || "team member"}`,
        lead_id:     lead.id,
        created_by:  profile?.id,
        status:      "done",
      }).then(() => {}).catch(() => {});
      ...
      toast.success(`${lead.company_name || lead.contact_name} moved back to Pipeline`);
      onClose();
    } catch (e) {
      toast.error("Failed: " + (e.message || "Unknown error"));
    } finally {
  ...
  return (
    <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <motion.div className="modal-box" initial={{ opacity: 0, scale: 0.94, y: 16 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.94, y: 16 }} transition={{ type: "spring", damping: 22, stiffness: 280 }} style={{ maxWidth: 420 }}>
        <div style={{ padding: "16px 24px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 36, height: 36, borderRadius: 10, background: "rgba(99,102,241,0.1)", border: "1.5px solid rgba(99,102,241,0.25)", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <ArrowRightLeft size={16} style={{ color: "#6366F1" }} />
            </div>
            <div>
              <h2 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: "var(--text)" }}>Move Back to Pipeline</h2>
              <p style={{ margin: 0, fontSize: 11.5, color: "var(--text-muted)" }}>This lead will return to the Pre-Sales Pipeline</p>
            </div>
          </div>
          <button onClick={onClose} className="btn-ghost" style={{ padding: 4 }}><X size={18} /></button>
        </div>
        <div style={{ padding: 24, display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={{ padding: "12px 16px", background: "rgba(99,102,241,0.06)", borderRadius: 10, border: "1px solid rgba(99,102,241,0.18)" }}>
            <div style={{ fontSize: 13.5, fontWeight: 700, color: "var(--text)", marginBottom: 3 }}>{lead.company_name}</div>
            {lead.contact_name && <div style={{ fontSize: 12, color: "var(--text-muted)" }}>{lead.contact_name}</div>}
          </div>
          <div style={{ padding: "10px 14px", background: "rgba(245,158,11,0.07)", borderRadius: 8, border: "1px solid rgba(245,158,11,0.2)", fontSize: 12.5, color: "#92400E" }}>
            ...
            <ul style={{ margin: "5px 0 0", paddingLeft: 16, display: "flex", flexDirection: "column", gap: 3 }}>
              <li>Lead moves to Pipeline with stage "New Prospect"</li>
              <li>All contact info, notes and activities are preserved</li>
              <li>Action is logged in the activity timeline</li>
            </ul>
          </div>
          <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
            <button type="button" className="btn-secondary" onClick={onClose}>Cancel</button>
            <button type="button" disabled={saving} onClick={handleConfirm}
              style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 18px", borderRadius: 8, border: "none", background: "#6366F1", color: "#fff", fontFamily: "inherit", fontSize: 13.5, fontWeight: 700, cursor: saving ? "not-allowed" : "pointer", opacity: saving ? 0.7 : 1 }}>
              <ArrowRightLeft size={13} /> {saving ? "Moving..." : "Move to Pipeline"}
            </button>
          </div>
        </div>
      </motion.div>
...
function InlineSourceSelect({ value, sources, onSave, disabled }) {
  const [open, setOpen] = useState(false);
  const ref = useRef();
  useEffect(() => {
    if (!open) return;
  ...
  return (
    <div ref={ref} style={{ position: "relative", display: "inline-block" }} onClick={(e) => e.stopPropagation()}>
      <button
        ...
        style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "2px 7px", borderRadius: 12, background: open ? "rgba(99,102,241,0.14)" : "rgba(99,102,241,0.08)", color: "#6366F1", fontWeight: 600, fontSize: 11, border: "1px solid rgba(99,102,241,0.18)", cursor: disabled ? "default" : "pointer", fontFamily: "inherit" }}
      >
        {cur ? (
          <>{(cur.key || cur.value) === "google" && <GoogleLogo size={11} />}{cur.label}</>
        ) : <span style={{ color: "var(--text-muted)", fontWeight: 400 }}>—</span>}
        {!disabled && <ChevronDown size={9} style={{ opacity: 0.55 }} />}
      </button>
      {open && (
        <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, zIndex: 400, background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 10, boxShadow: "0 8px 28px rgba(0,0,0,0.14)", padding: "4px 0", minWidth: 175, maxHeight: 240, overflowY: "auto" }} className="custom-scroll">
          ...
          {sources.map((s) => {
            ...
            return (
              <button key={k} onClick={() => { onSave(k); setOpen(false); }}
                style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", padding: "6px 12px", border: "none", background: isSel ? "rgba(99,102,241,0.08)" : "transparent", cursor: "pointer", textAlign: "left", fontSize: 12.5, color: isSel ? "#6366F1" : "var(--text)", fontWeight: isSel ? 700 : 400, fontFamily: "inherit" }}
                onMouseEnter={(e) => e.currentTarget.style.background = isSel ? "rgba(99,102,241,0.12)" : "var(--surface-2)"}
                onMouseLeave={(e) => e.currentTarget.style.background = isSel ? "rgba(99,102,241,0.08)" : "transparent"}
              >
                {k === "google" && <GoogleLogo size={13} />}
                {s.label}
                {isSel && <span style={{ marginLeft: "auto", fontSize: 10, color: "#6366F1" }}>✓</span>}
              </button>
            );
          })}
        </div>
...
// ─── Bulk Action Bar ──────────────────────────────────────────────────────────
function BulkActionBar({ count, teamMembers, onAssign, onStageChange, onDelete, onClear, canAssign = true, canDeleteRec = true }) {
...
export default function Leads() {
  ...
  const [leadHighlightActivityId, setLeadHighlightActivityId] = useState(null);
  const [convertLead, setConvertLead]     = useState(null);
  const [revertPipelineLead, setRevertPipelineLead] = useState(null);
  ...
  const pendingLeadActivityRef = useRef(searchParams.get("activity"));
  ...
  const { data: leadsData, isPending: leadsIsPending } = useQuery({
    queryKey: ["leads", search, viewFilter, profile?.id],
    queryFn: () => leadsService.getAll({ search, assignedTo: assignedToParam, profileId: profile?.id, role: profile?.role }),
    enabled: !!profile?.id,
    retry: 1,
  });
  const isLoading = leadsIsPending;
  ...
  const inlineUpdateMutation = useMutation({
    mutationFn: ({ id, payload }) => leadsService.update(id, payload),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["leads"] }); toast.success("Updated"); },
    onError: (e) => toast.error(e.message),
  });
  ...
  // Auto-open detail panel when navigated from a notification link (/leads?selected=<id>&activity=<id>)
  useEffect(() => {
    ...
    if (match) {
      ...
      if (pendingLeadActivityRef.current) {
        setLeadHighlightActivityId(pendingLeadActivityRef.current);
        pendingLeadActivityRef.current = null;
      }
  ...
  // ── Pagination ────────────────────────────────────────────────────────────
  const totalLeadsPages  = Math.max(1, Math.ceil(leads.length / PAGE_SIZE));
  const safeLeadsPage    = Math.min(leadsPage, totalLeadsPages);
  const paginatedLeads   = leads.slice((safeLeadsPage - 1) * PAGE_SIZE, safeLeadsPage * PAGE_SIZE);
  // Reset to page 1 whenever filters produce a new leads array
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <div style={{ padding: "8px 24px", display: "flex", alignItems: "center", gap: 8, borderBottom: "1px solid var(--border)" }}>
        ...
        {/* Pagination — right of search, before date filter */}
        <Pagination
          compact
          currentPage={safeLeadsPage}
        ...
        <AnimatePresence>
          {selectedIds.size > 0 && (
            <BulkActionBar
              ...
              canAssign={isManager}
              canDeleteRec={canDelete}
            />
          )}
        </AnimatePresence>

        {isLoading ? (
          <SkeletonTable cols={8} rows={9} hasCheckbox={true} />
        ) : leads.length === 0 ? (
                    ...
                    return (
                      ...
                      >
                          ...
                          <td>
                            {extra.country ? (
                              <span style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 12, color: "var(--text-2)" }}>
                                <FlagImg code={extra.country} size={18} />
                        ...
                        {isVisible("source") && (
                          <td onClick={(e) => e.stopPropagation()}>
                            <InlineSourceSelect
                              ...
                              disabled={l.is_locked && !isSalesHead}
                              onSave={(v) => inlineUpdateMutation.mutate({ id: l.id, payload: { source: v } })}
                            />
                          </td>
                        )}
                        {isVisible("website") && (
                        ...
                        {isVisible("linkedin") && (
                          <td>
                            {(() => {
                              const li = parseJSON(l.other_notes).company_linkedin || parseJSON(l.other_notes).linkedin_url;
                              if (!li) return <span style={{ color: "var(--text-muted)", fontSize: 11 }}>—</span>;
                              const display = li.replace(/^https?:\/\/(www\.)?/i, "").replace(/\/$/, "");
                              return (
                                <a href={li.startsWith("http") ? li : `https://${li}`} target="_blank" rel="noopener noreferrer" title={li}
                                  onClick={(ev) => ev.stopPropagation()}
                        ...
                        {isVisible("status") && (
                          <td onClick={(e) => e.stopPropagation()}>
                            <InlineStageSelect
                              ...
                              disabled={l.is_locked && !isSalesHead}
                              onSave={(v) => inlineUpdateMutation.mutate({ id: l.id, payload: { stage: v } })}
                            />
                          </td>
                        )}
                        {isVisible("date") && (
                              ...
                              return (
                                <>
                                  ...
                                  <motion.button
                                    ...
                                    title={!leadHasContact(l) ? "Add Email or Phone before converting" : "Convert to Deal"}>
                                    <ArrowRight size={11} /> Deal
                                  </motion.button>
                                  {/* Revert to Pipeline: admin only */}
                                  {isSalesHead && (
                                    <motion.button
                                      style={{ display: "flex", alignItems: "center", gap: 4, padding: "4px 10px", borderRadius: 7, fontSize: 11.5, fontWeight: 700, background: "rgba(99,102,241,0.1)", color: "#6366F1", border: "1px solid rgba(99,102,241,0.28)", cursor: "pointer", fontFamily: "inherit" }}
                                      onClick={(e) => { e.stopPropagation(); setRevertPipelineLead(l); }}
                                      whileHover={{ scale: 1.04, background: "rgba(99,102,241,0.18)" }} whileTap={{ scale: 0.94 }}
                                      title="Move this lead back to Pipeline">
                                      <ArrowRightLeft size={11} /> Pipeline
                                    </motion.button>
                                  )}
      ...
      <AnimatePresence>
        {selectedLead && (
          <LeadDetailPanel
            lead={selectedLead}
            onClose={() => { setSelectedLead(null); setLeadHighlightActivityId(null); }}
            onEdit={(lead) => { setSelectedLead(null); setEditLead(lead); }}
            onConvert={(lead) => { setSelectedLead(null); setConvertLead(lead); }}
            onRevertPipeline={(lead) => { setSelectedLead(null); setRevertPipelineLead(lead); }}
            highlightActivityId={leadHighlightActivityId}
            onHighlightConsumed={() => setLeadHighlightActivityId(null)}
          />
        )}
        {(showForm || editLead) && (
          <LeadModal
        ...
        {revertPipelineLead && (
          <RevertToPipelineModal lead={revertPipelineLead} onClose={() => setRevertPipelineLead(null)} />
        )}
        {proposalLead && (
