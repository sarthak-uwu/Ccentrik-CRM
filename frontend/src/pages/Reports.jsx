...
import {
  ...
  Share2, Megaphone, UsersRound, Loader2,
} from "lucide-react";
...
export default function Reports() {
  ...
  const handlePdf = async () => {
    ...
    try {
      ...
      doc.setFontSize(18);
      doc.text("Reports & Analytics", M, 14);
      doc.setFont("helvetica", "normal");
      doc.setFontSize(9.5);
      doc.text(dateLabel, M, 22);
      doc.text(`${computed.totalLeads} leads  ·  ${computed.teamPerf.length} team members`, M, 29);
      doc.setFontSize(8);
      ...
      const kpis = [
        { label: "Total Leads",  value: String(computed.totalLeads),       color: ACCENT },
        { label: "Won",          value: String(computed.wonLeads),         color: [16, 185, 129] },
        { label: "Lost",         value: String(computed.lostLeads),        color: [239, 68, 68] },
        { label: "Conv. Rate",   value: `${computed.convRate}%`,           color: [16, 185, 129] },
        { label: "Hot Leads",    value: String(computed.hotLeads),         color: [239, 68, 68] },
        { label: "Revenue",      value: formatCompact(computed.totalRevenue), color: [16, 185, 129] },
        { label: "Open Deals",   value: String(computed.openDeals),        color: ACCENT },
      ];
      const KCOLS = 7, KGAP = 2;
      const BOX_W = (CW - KGAP * (KCOLS - 1)) / KCOLS, BOX_H = 16;
      kpis.forEach((kpi, i) => {
        const bx = M + i * (BOX_W + KGAP);
        doc.setFillColor(247, 247, 255);
        doc.roundedRect(bx, y, BOX_W, BOX_H, 1.5, 1.5, "F");
        doc.setFillColor(...kpi.color);
        doc.rect(bx, y, BOX_W, 1.8, "F");
        doc.setFont("helvetica", "bold");
        doc.setFontSize(11);
        doc.setTextColor(...kpi.color);
        doc.text(kpi.value, bx + BOX_W / 2, y + 9.5, { align: "center" });
        doc.setFont("helvetica", "normal");
        doc.setFontSize(6);
        doc.setTextColor(110, 110, 130);
        doc.text(kpi.label, bx + BOX_W / 2, y + 13.5, { align: "center" });
      });
      y += BOX_H + 9;

      // Team Performance
      if (computed.teamPerf.length > 0) {
        drawSection("Team Performance");
        autoTable(doc, {
          startY: y,
          head: [["Name", "Role", "Leads", "Won", "Activities", "Conv %", "Revenue"]],
          body: computed.teamPerf.map((p) => [
            p.name, p.role, String(p.leads), String(p.won),
            String(p.activities), `${p.convRate}%`, formatCompact(p.revenue),
          ]),
      ...
      // Stage Funnel + Source side by side
      if (y > 220) { doc.addPage(); y = M; }
      const halfW = (CW - 8) / 2;

      drawSection("Lead Stage Funnel");
      autoTable(doc, {
        startY: y,
        head: [["Stage", "Count", "%"]],
        body: computed.stageFunnel.map((s) => [
          s.name, String(s.value),
          `${computed.totalLeads > 0 ? Math.round((s.value / computed.totalLeads) * 100) : 0}%`,
        ]),
      ...
      const funnelEndY = doc.lastAutoTable.finalY;
      ...
      doc.setFontSize(7.5);
      doc.setTextColor(120, 120, 140);
      doc.text("LEAD SOURCE BREAKDOWN", M + halfW + 8, srcStartY - 5);
      autoTable(doc, {
        startY: srcStartY,
        head: [["Source", "Count", "%"]],
        body: computed.sourceData.slice(0, 8).map((s) => [
          s.name, String(s.value),
          `${computed.totalLeads > 0 ? Math.round((s.value / computed.totalLeads) * 100) : 0}%`,
        ]),
        styles: { fontSize: 7.5, cellPadding: 2.5 },
      ...
      y = Math.max(funnelEndY, doc.lastAutoTable.finalY) + 8;

      // Monthly Trend
      if (y > 220) { doc.addPage(); y = M; }
      drawSection("6-Month Lead Trend");
      autoTable(doc, {
        startY: y,
        head: [["Month", "Total Leads", "Won"]],
        body: computed.monthlyLeads.map((m) => [m.month, String(m.total), String(m.won)]),
        styles: { fontSize: 7.5, cellPadding: 2.5 },
      ...
      if (computed.funnelConversions.length > 0) {
        if (y > 220) { doc.addPage(); y = M; }
        drawSection("Funnel Conversion Rates");
        autoTable(doc, {
          startY: y,
          head: [["From", "→ To", "From Count", "To Count", "Rate"]],
          body: computed.funnelConversions.map((f) => [
            f.from, f.to, String(f.fromCount), String(f.toCount), `${f.rate}%`,
          ]),
          styles: { fontSize: 7.5, cellPadding: 2.5 },
      ...
      // Key Insights
      if (computed.aiInsights.length > 0) {
        if (y > 245) { doc.addPage(); y = M; }
        drawSection("Key Insights");
        computed.aiInsights.forEach((insight, i) => {
          if (y > 270) { doc.addPage(); y = M + 5; }
          doc.setFont("helvetica", "normal");
          doc.setFontSize(8.5);
          doc.setTextColor(60, 60, 80);
          doc.text(`${i + 1}.  ${insight}`, M + 3, y);
          y += 7;
        });
      }
      ...
      for (let p = 1; p <= pages; p++) {
        ...
        doc.text(`Ccentrik CRM  ·  Reports & Analytics  ·  ${dateLabel}`, M, 293);
        doc.text(`Page ${p} of ${pages}`, W - M, 293, { align: "right" });
      }

      doc.save(`CcentrikCRM_Report_${format(new Date(), "yyyyMMdd_HHmm")}.pdf`);
    } finally {
      setPdfLoading(false);
    }
  };
