...
import {
  TrendingUp, Users, Zap, DollarSign, Award, Target, Printer,
  Filter, Calendar, SlidersHorizontal, Brain,
  BarChart2, PieChart as PieChartIcon, Flame,
} from "lucide-react";
  ...
  return (
    <div style={{ padding: 24, overflowY: "auto", height: "100%" }}>
      ...
      <div style={{ marginBottom: 20, display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
        <div>
          ...
          <p style={{ margin: 0, fontSize: 13, color: "var(--text-2)" }}>
            {dateLabel} · {computed.totalLeads} leads · {computed.teamPerf.length} team members · AI-powered insights
          </p>
        </div>
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <button className="btn-secondary no-print" onClick={() => window.print()} style={{ display: "flex", alignItems: "center", gap: 7 }}>
            <Printer size={14} /> PDF
          </button>
        </div>
      </div>
      ...
      {computed.aiInsights.length > 0 && (
        <div className="card-ai" style={{ marginBottom: 20, padding: "14px 18px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
            <Brain size={15} style={{ color: "var(--violet)" }} />
            <span style={{ fontSize: 12.5, fontWeight: 700, color: "var(--violet)" }}>AI Insights</span>
          </div>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            {computed.aiInsights.map((insight, i) => (
              <div key={i} className="ai-suggestion" style={{ flex: "1 1 200px" }}>
                {insight}
              </div>
            ))}
          </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(190px, 1fr))", gap: 14, marginBottom: 24 }}>
        <KpiCard icon={Zap}       label="Total Leads"   value={computed.totalLeads}  color="#1B76D3" />
        <KpiCard icon={Award}     label="Won Leads"      value={computed.wonLeads}    sub={`${computed.convRate}% conversion`} color="#10B981" />
        <KpiCard icon={Target}    label="Lost Leads"     value={computed.lostLeads}   color="#EF4444" />
        <KpiCard icon={DollarSign}label="Won Revenue"    value={computed.totalRevenue > 0 ? `₹${(computed.totalRevenue / 100000).toFixed(1)}L` : "₹0"} color="#F59E0B" />
        <KpiCard icon={TrendingUp}label="Open Deals"     value={computed.openDeals}   color="#3B82F6" />
        <KpiCard icon={Users}     label="Hot Leads"      value={computed.hotLeads}    color="#EF4444" />
      </div>
      ...
      <div className="reports-chart-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18, marginBottom: 18 }}>
        <ChartCard title="Monthly Leads — Total vs Won (6 months)" icon={BarChart2}>
        ...
        <ChartCard title="Leads by Source" icon={PieChartIcon}>
      ...
      <div className="reports-chart-grid" style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr", gap: 18, marginBottom: 18 }}>
        {/* Stage funnel */}
        <ChartCard title="Lead Stage Distribution" icon={Target}>
        ...
        {/* Temperature */}
        <ChartCard title="Lead Temperature" icon={Flame} iconColor="#EF4444">
        ...
        {/* Activity breakdown */}
        <ChartCard title="Activity Breakdown" icon={Zap} iconColor="#8B5CF6">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 18, marginBottom: 18 }}>
        ...
        <ChartCard title="Revenue Forecast — Won + 3-Month Projection (₹L)" icon={TrendingUp} iconColor="#10B981">
          <ResponsiveContainer width="100%" height={220}>
            <ComposedChart data={computed.forecastData} barGap={4}>
              ...
              <Tooltip content={<CrmTooltip />} formatter={(v) => `₹${v}L`} />
        ...
        <ChartCard title="Funnel Conversion Rates" icon={Target}>
              ...
              <tbody>
                {computed.teamPerf.map((p) => (
                  <tr key={p.id}>
                    ...
                    <td>{p.activities}</td>
                    <td style={{ fontWeight: 600, color: "#10B981" }}>{p.revenue > 0 ? `₹${(p.revenue / 100000).toFixed(1)}L` : "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
