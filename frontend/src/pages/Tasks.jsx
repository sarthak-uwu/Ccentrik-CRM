...
// ─── Kanban Card ──────────────────────────────────────────────────────────────
function TaskCard({ task, onEdit, onDelete, onToggle, isDragging }) {
  const overdue = task.due_date && isPast(new Date(task.due_date)) && task.status !== "done";
  return (
    <div className="kanban-card" style={{ opacity: isDragging ? 0.4 : 1 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
        ...
        {task.due_date && (
          <span style={{ fontSize: 11, color: overdue ? "var(--red)" : isToday(new Date(task.due_date)) ? "var(--amber)" : "var(--text-muted)", display: "flex", alignItems: "center", gap: 2 }}>
...
function DraggableTask({ task, onEdit, onDelete, onToggle }) {
  ...
  return (
    <div ref={setNodeRef} style={style} {...listeners} {...attributes}>
      <TaskCard task={task} onEdit={onEdit} onDelete={onDelete} onToggle={onToggle} isDragging={isDragging} />
    </div>
  );
}

function DroppableCol({ status, tasks, onEdit, onDelete, onToggle }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "7px 7px 10px", display: "flex", flexDirection: "column", gap: 6, minHeight: 60 }}>
        {tasks.map((t) => <DraggableTask key={t.id} task={t} onEdit={onEdit} onDelete={onDelete} onToggle={onToggle} />)}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <div style={{ padding: "12px 20px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 10, background: "var(--surface)", flexWrap: "wrap" }}>
        ...
        <button className="btn-primary" onClick={() => { setEditTask(null); setShowForm(true); }} style={{ height: 36 }}>
        ...
        {isLoading ? (
          ...
          >
            <div style={{ display: "flex", gap: 12, padding: "14px 20px", overflowX: "auto", height: "100%", alignItems: "flex-start" }}>
              {STATUSES.map((status) => (
                <DroppableCol key={status.key} status={status} tasks={tasks.filter((t) => t.status === status.key)} onEdit={setEditTask} onDelete={(id) => { if (confirm("Delete task?")) deleteMutation.mutate(id); }} onToggle={handleToggle} />
              ))}
            </div>
            <DragOverlay>
              {activeTask && <div style={{ transform: "rotate(2deg)", opacity: 0.9, width: 220 }}><TaskCard task={activeTask} onEdit={() => {}} onDelete={() => {}} onToggle={() => {}} /></div>}
            </DragOverlay>
          </DndContext>

        ) : view === "calendar" ? (
          <CalendarView tasks={tasks} onEdit={setEditTask} onToggle={handleToggle} />
              ...
              <table className="crm-table">
                ...
                <tbody>
                  {tasks.length === 0 ? (
                    ...
                    return (
                      <tr key={t.id} style={{ background: selected.includes(t.id) ? "var(--accent-light)" : "transparent" }}>
                        ...
                        <td style={{ fontSize: 12.5, color: "var(--text-2)" }}>{t.assigned_profile?.full_name?.split(" ")[0] || "—"}</td>
