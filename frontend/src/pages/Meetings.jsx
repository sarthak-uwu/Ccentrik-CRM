...
const generateJitsiRoom = (id) =>
  `https://meet.jit.si/Ccentrik-${id.slice(0, 8).toUpperCase()}-${Math.random().toString(36).slice(2, 6).toUpperCase()}`;

// Platform config used by form and invite email
const PLATFORMS = [
  { key: "google_meet", label: "Google Meet",      icon: Video,    color: "#34A853", bg: "rgba(52,168,83,0.1)"    },
  { key: "teams",       label: "Microsoft Teams",  icon: Video,    color: "#6264A7", bg: "rgba(98,100,167,0.1)"   },
  { key: "jitsi",       label: "Jitsi Meet",       icon: Video,    color: "#EC5800", bg: "rgba(236,88,0,0.1)"     },
  { key: "in_person",   label: "In-Person",        icon: MapPin,   color: "#10B981", bg: "rgba(16,185,129,0.1)"   },
];
...
function MeetingFormModal({ meeting, onClose, onSave, teamMembers = [], leads = [], deals = [], pipeline = [], allMeetings = [], codeMap = {}, reuseFrom = null }) {
  ...
  const applyReuseMeeting = useCallback((src) => {
    ...
    const srcPlatform = src.meeting_type || (src.mode === "offline" ? "in_person" : "google_meet");
    setMode(src.mode || "online");
    setPlatform(srcPlatform);
  ...
  const handleFormSubmit = async (data) => {
    ...
    await onSave({
      ...
      mode:            platform === "in_person" ? "offline" : "online",
      meeting_purpose: finalPurpose,
      meeting_link:    mode === "online" ? (data.meeting_link || null) : null,
      location:        mode === "offline" ? (() => {
        const parts = [data.location, data.location_city, data.location_state, data.location_country, data.location_pincode].filter(Boolean);
        return parts.length ? parts.join(", ") : null;
      })() : null,
      maps_url:        mode === "offline" && data.location ? `https://maps.google.com/?q=${encodeURIComponent([data.location, data.location_city, data.location_country].filter(Boolean).join(", "))}` : null,
      ...
      <motion.div className="modal-box" initial={{ opacity: 0, scale: 0.94, y: 16 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.94, y: 16 }} transition={{ type: "spring", damping: 22, stiffness: 280 }} style={{ maxWidth: 900, width: "96vw", maxHeight: "94vh", overflowY: "auto" }}>
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            ...
            {/* ── § 4: Meeting Platform ── */}
            <SectionHeader icon={Globe} label="Meeting Platform" />
                ...
                {PLATFORMS.map((p) => {
                  ...
                  return (
                    <button key={p.key} type="button"
                      onClick={() => {
                        setPlatform(p.key);
                        setMode(p.key === "in_person" ? "offline" : "online");
                      }}
                      style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, padding: "12px 8px", borderRadius: 10, border: `2px solid ${isActive ? p.color : "var(--border)"}`, cursor: "pointer", fontFamily: "inherit", fontSize: 12, fontWeight: 600, background: isActive ? p.bg : "var(--surface-2)", color: isActive ? p.color : "var(--text-muted)", transition: "all 0.15s" }}>
                      <Icon size={16} />
                      {p.label}
                    </button>
                  );
            ...
            {platform !== "in_person" ? (
              <div style={{ gridColumn: "1 / -1" }}>
                <label className="crm-label" style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <span>Meeting Link</span>
                  {platform === "jitsi" && (
                    <button type="button"
                      onClick={() => {
                        const room = generateJitsiRoom(meeting?.id || Date.now().toString(36));
                        setValue("meeting_link", room);
                        setAutoJitsi(true);
                      }}
                      style={{ fontSize: 11, fontWeight: 700, color: "#EC5800", background: "rgba(236,88,0,0.08)", border: "1px solid rgba(236,88,0,0.25)", borderRadius: 6, padding: "3px 10px", cursor: "pointer", fontFamily: "inherit" }}>
                      &#9734; Auto-Generate Jitsi Room
                    </button>
                  )}
                  {platform === "teams" && (
                    <button type="button" disabled={platformLoading}
                      onClick={async () => {
                        setPlatformLoading(true);
                        try {
                          const token = await auth.currentUser?.getIdToken();
                          const r = await fetch(`${API}/api/meetings/create-teams`, {
                            method: "POST",
                            headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
                            body: JSON.stringify({ title: watch("title") || "Meeting", startTime: watch("start_time"), endTime: watch("end_time") }),
                          });
                          const data = await r.json();
                          if (data.joinUrl) { setValue("meeting_link", data.joinUrl); toast.success("Teams meeting created!"); }
                          else toast(data.message || "Set TEAMS_CLIENT_ID env var to auto-create Teams meetings", { icon: "ℹ️" });
                        } catch { toast("Could not reach server", { icon: "⚠️" }); }
                        setPlatformLoading(false);
                      }}
                      style={{ fontSize: 11, fontWeight: 700, color: "#6264A7", background: "rgba(98,100,167,0.08)", border: "1px solid rgba(98,100,167,0.25)", borderRadius: 6, padding: "3px 10px", cursor: platformLoading ? "not-allowed" : "pointer", fontFamily: "inherit", opacity: platformLoading ? 0.6 : 1 }}>
                      {platformLoading ? "Creating…" : "&#9734; Create Teams Meeting"}
                    </button>
                  )}
                  {platform === "google_meet" && (
                    <button type="button" disabled={platformLoading}
                      onClick={async () => {
                        ...
                        try {
                          const token = await auth.currentUser?.getIdToken();
                          const r = await fetch(`${API}/api/meetings/create-meet`, {
                            method: "POST",
                            headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
                            body: JSON.stringify({ title: watch("title") || "Meeting", startTime: watch("start_time"), endTime: watch("end_time") }),
                          });
                          const data = await r.json();
                          if (data.meetLink) { setValue("meeting_link", data.meetLink); toast.success("Google Meet link created!"); }
                          else toast(data.message || "Set GOOGLE_CLIENT_ID env var or paste a Meet link manually", { icon: "ℹ️" });
                        } catch { toast("Could not reach server", { icon: "⚠️" }); }
                        setPlatformLoading(false);
                      }}
                      style={{ fontSize: 11, fontWeight: 700, color: "#34A853", background: "rgba(52,168,83,0.08)", border: "1px solid rgba(52,168,83,0.25)", borderRadius: 6, padding: "3px 10px", cursor: platformLoading ? "not-allowed" : "pointer", fontFamily: "inherit", opacity: platformLoading ? 0.6 : 1 }}>
                      {platformLoading ? "Creating…" : "&#9734; Create Google Meet"}
                    </button>
                  )}
                </label>
                <input className="crm-input" {...register("meeting_link")}
                  placeholder={platform === "jitsi" ? "https://meet.jit.si/Ccentrik-XXXX" : platform === "teams" ? "https://teams.microsoft.com/l/meetup-join/…" : "https://meet.google.com/abc-defg-hij"} />
              ...
              <div style={{ gridColumn: "1 / -1" }}>
                <label className="crm-label">Meeting Address</label>
                <input className="crm-input" {...register("location")} placeholder="Street address, building name, floor…" style={{ marginBottom: 8 }} />
                {/* Address sub-fields stored as metadata in location string */}
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
                  <input className="crm-input" {...register("location_city")} placeholder="City" />
                  <input className="crm-input" {...register("location_state")} placeholder="State" />
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
                  <input className="crm-input" {...register("location_country")} placeholder="Country" />
                  <input className="crm-input" {...register("location_pincode")} placeholder="Pincode / ZIP" />
                </div>
                {/* Google Maps preview */}
                {(() => {
                  ...
                  const fullAddr = [loc, city, country].filter(Boolean).join(", ");
                  if (!fullAddr) return null;
                  const mapsUrl = `https://maps.google.com/?q=${encodeURIComponent(fullAddr)}`;
                  return (
                    <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 12px", borderRadius: 8, background: "rgba(59,130,246,0.06)", border: "1px solid rgba(59,130,246,0.2)" }}>
                      <MapPin size={12} style={{ color: "#3B82F6", flexShrink: 0 }} />
                      <span style={{ fontSize: 11.5, color: "var(--text-2)", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{fullAddr}</span>
                      <a href={mapsUrl} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()}
                        style={{ fontSize: 11, color: "#3B82F6", textDecoration: "none", fontWeight: 600, flexShrink: 0, display: "flex", alignItems: "center", gap: 3 }}>
...
export default function Meetings() {
  ...
  const downloadICS = (m) => {
    ...
    const uid   = `${m.id}@ccentrik.com`;
    const start = fmtDT(m.start_time);
    const end   = m.end_time ? fmtDT(m.end_time) : fmtDT(new Date(new Date(m.start_time).getTime()+3600000));
    const lines = [
      "BEGIN:VCALENDAR","VERSION:2.0","PRODID:-//Ccentrik CRM//Meeting//EN","CALSCALE:GREGORIAN","METHOD:PUBLISH",
      "BEGIN:VEVENT",
      `UID:${uid}`,`DTSTAMP:${fmtDT(new Date())}`,`DTSTART:${start}`,`DTEND:${end}`,
      `SUMMARY:${esc(m.title)}`,
      m.location   ? `LOCATION:${esc(m.location)}`   : null,
      m.meeting_link ? `URL:${m.meeting_link}` : null,
      m.agenda     ? `DESCRIPTION:${esc(m.agenda)}` : null,
      "STATUS:CONFIRMED","BEGIN:VALARM","TRIGGER:-PT30M","ACTION:DISPLAY",`DESCRIPTION:Upcoming: ${esc(m.title)}`,"END:VALARM",
      "END:VEVENT","END:VCALENDAR",
    ].filter(Boolean).join("\r\n");
    const blob = new Blob([lines], { type: "text/calendar;charset=utf-8" });
    const a = Object.assign(document.createElement("a"), { href: URL.createObjectURL(blob), download: `${(m.title||"meeting").replace(/[^a-z0-9]/gi,"_")}.ics` });
  ...
  const sendMeetingInvite = async (data, isUpdate = false) => {
    ...
    try {
      ...
      const payload = {
        ...
        hostTitle:      profile?.designation || null,
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["meetings"] }); toast.success("Meeting updated"); setEditMeeting(null); },
    onError: (e) => toast.error(e.message),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["meetings"] }); toast.success("Meeting removed"); },
    onError: (e) => toast.error(e.message),
  });
                        ...
                        <motion.tr key={m.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ delay: i * 0.02, duration: 0.16 }}>
                          ...
                          <td>
                            <div style={{ display: "flex", alignItems: "center", gap: 3, justifyContent: "flex-end" }}>
                              ...
                              {/* Download ICS */}
                              <motion.button onClick={() => downloadICS(m)} title="Download .ics calendar file" className="btn-ghost" style={{ padding: "4px 6px", color: "#3B82F6" }} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
