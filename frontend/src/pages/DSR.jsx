...
import {
  ...
  DollarSign, Star, BarChart3, Bell,
} from "lucide-react";
import {
...
export default function DSRPage() {
  ...
  const { data: acts = [], isFetching } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("activities").select("id, type, title, description, created_at, status, due_date")
  ...
  const filteredActs = useMemo(() => {
    return acts.filter((a) => {
      ...
      if (actSearch && !a.title?.toLowerCase().includes(actSearch.toLowerCase()) && !a.description?.toLowerCase().includes(actSearch.toLowerCase())) return false;
      return true;
    });
  ...
  const handleExportPDF = useCallback(async () => {
    ...
    doc.setFontSize(18);
    doc.text("Daily Sales Report", M, 14);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9.5);
    doc.text(periodLabel, M, 22);
    const pdfUser = selectedUser?.full_name || profile?.full_name || "";
    const pdfRole = (selectedUser?.role || profile?.role || "").replace(/_/g, " ");
    doc.text(`${pdfUser}  ·  ${pdfRole}`, M, 29);
    doc.setFontSize(8);
    doc.text(`Generated: ${format(new Date(), "dd MMM yyyy, h:mm a")}`, W - M, 29, { align: "right" });
    y = 44;
    ...
    drawSection("Summary");
    const kpis = [
      { label: "Activities",  value: String(totalActs),       color: ACCENT },
      { label: "Acts Done",   value: String(actsDone),        color: [16, 185, 129] },
      { label: "Tasks Done",  value: String(tasksCompleted),  color: [16, 185, 129] },
      { label: "Pending",     value: String(pendingTasks),    color: [245, 158, 11] },
      { label: "Overdue",     value: String(overdueTasks),    color: [239, 68, 68] },
      { label: "Deals Moved", value: String(dealsUpdated),    color: ACCENT },
      { label: "New Leads",   value: String(newLeads.length), color: [6, 182, 212] },
      { label: "Won",         value: String(dealsWon),        color: [16, 185, 129] },
      { label: "Revenue",     value: fmtCurrency(revenueWon), color: [16, 185, 129] },
      { label: "Score",       value: `${score}/100`,          color: score >= 70 ? [16, 185, 129] : score >= 40 ? [245, 158, 11] : [239, 68, 68] },
    ];
    const COLS = 5, GAP = 3;
    const BOX_W = (CW - GAP * (COLS - 1)) / COLS;
    const BOX_H = 17;
    kpis.forEach((kpi, i) => {
      const col = i % COLS, row = Math.floor(i / COLS);
      const bx = M + col * (BOX_W + GAP), by = y + row * (BOX_H + GAP);
      doc.setFillColor(247, 247, 255);
      doc.roundedRect(bx, by, BOX_W, BOX_H, 1.5, 1.5, "F");
      doc.setFillColor(...kpi.color);
      doc.rect(bx, by, BOX_W, 1.8, "F");
      doc.setFont("helvetica", "bold");
      doc.setFontSize(13);
      doc.setTextColor(...kpi.color);
      doc.text(kpi.value, bx + BOX_W / 2, by + 10, { align: "center" });
      doc.setFont("helvetica", "normal");
      doc.setFontSize(6.5);
      doc.setTextColor(110, 110, 130);
      doc.text(kpi.label, bx + BOX_W / 2, by + 14.5, { align: "center" });
    });
    y += Math.ceil(kpis.length / COLS) * (BOX_H + GAP) + 7;

    // Communication mix
    drawSection("Communication Mix");
    const commEntries = Object.entries(CORE_TYPES);
    const COMM_W = (CW - GAP * (commEntries.length - 1)) / commEntries.length;
    const COMM_H = 13;
    commEntries.forEach(([key, cfg], i) => {
      const count = typeCounts[key] || 0;
      const rgb = hex2rgb(cfg.color);
      const bx = M + i * (COMM_W + GAP);
      doc.setFillColor(248, 248, 255);
      doc.roundedRect(bx, y, COMM_W, COMM_H, 1.5, 1.5, "F");
      if (count > 0) {
        doc.setDrawColor(...rgb);
        doc.setLineWidth(0.25);
        doc.roundedRect(bx, y, COMM_W, COMM_H, 1.5, 1.5, "S");
      }
      doc.setFont("helvetica", "bold");
      doc.setFontSize(10);
      doc.setTextColor(count > 0 ? rgb[0] : 160, count > 0 ? rgb[1] : 160, count > 0 ? rgb[2] : 160);
      doc.text(String(count), bx + COMM_W / 2, y + 7.5, { align: "center" });
      doc.setFont("helvetica", "normal");
      doc.setFontSize(5.5);
      doc.setTextColor(130, 130, 150);
      doc.text(cfg.short, bx + COMM_W / 2, y + 11.5, { align: "center" });
    });
    y += COMM_H + 9;

    // Activity log
    drawSection("Activity Log");
    if (acts.length === 0) {
      doc.setFont("helvetica", "italic");
      doc.setFontSize(9);
      doc.setTextColor(160, 160, 180);
      doc.text("No activities recorded for this period.", M, y + 4);
      y += 12;
    } else {
      const tsFmt = range === "daily" ? "h:mm a" : range === "weekly" ? "EEE, h:mm a" : "MMM d, h:mm a";
      autoTable(doc, {
        startY: y,
        head: [["#", "Date / Time", "Type", "Title", "Description", "Status"]],
        body: acts.map((a, i) => [
          String(i + 1),
          format(new Date(a.created_at), tsFmt),
          CORE_TYPES[resolveType(a.type)]?.label || resolveType(a.type),
          a.title || "—",
        ...
        styles: { fontSize: 7.5, cellPadding: { top: 3, bottom: 3, left: 3, right: 3 }, overflow: "linebreak", valign: "middle", textColor: [40, 40, 60] },
        ...
        didParseCell: (data) => {
          ...
          else if (v === "In Progress") data.cell.styles.textColor = [99, 102, 241];
    ...
    if (newLeads.length > 0) {
      ...
      drawSection("New Leads");
      autoTable(doc, {
        startY: y,
        head: [["Company", "Contact", "Stage", "Created"]],
        body: newLeads.map((l) => [
          l.company_name || "—",
          l.contact_name || "—",
        ...
        alternateRowStyles: { fillColor: [247, 254, 255] },
        margin: { left: M, right: M },
      });
    }
    ...
    for (let p = 1; p <= pages; p++) {
      ...
      doc.text(`Ccentrik CRM  ·  ${periodLabel}  ·  ${pdfUser}`, M, 293);
      doc.text(`Page ${p} of ${pages}`, W - M, 293, { align: "right" });
    }

    doc.save(`DSR_${pdfUser.replace(/\s+/g, "_")}_${format(new Date(), "yyyyMMdd_HHmm")}.pdf`);
  }, [acts, totalActs, actsDone, tasksCompleted, pendingTasks, overdueTasks, dealsUpdated,
      newLeads, dealsWon, revenueWon, score, typeCounts, periodLabel, selectedUser, profile, range]);
    ...
    <div style={{ padding: 24, maxWidth: 1400 }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 18, flexWrap: "wrap", gap: 12 }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <button onClick={handleExportPDF} style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 12px", borderRadius: 9, border: "1px solid var(--border)", background: "var(--surface)", color: "var(--text-2)", fontSize: 12, fontWeight: 500, cursor: "pointer" }}>
                  ...
                  {timelineGroups.map((group) => (
                    <div key={group.label} style={{ marginBottom: 18 }}>
                      ...
                      {group.acts.map((act, idx) => {
                        const k      = resolveType(act.type);
                        const cfg    = CORE_TYPES[k];
                        const done   = act.status === "done";
                        const isMeet = k === "meeting_person" || k === "meeting_virtual";
                        return (
                          <motion.div key={act.id}
                            initial={{ opacity: 0, x: -4 }} animate={{ opacity: 1, x: 0 }}
                            transition={{ delay: Math.min(idx * 0.02, 0.18), duration: 0.16 }}
                            style={{ display: "flex", alignItems: "center", gap: 12, padding: "9px 8px", borderBottom: "1px solid var(--border)", borderLeft: `2px solid ${done ? cfg.color : isMeet && !done ? "#EF4444" : cfg.color + "45"}`, paddingLeft: 10 }}>
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ fontSize: 13, fontWeight: 600, color: "var(--text)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{act.title || cfg.label}</div>
                              <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 2, display: "flex", alignItems: "center", gap: 5 }}>
                                <span style={{ padding: "1px 6px", borderRadius: 4, background: cfg.bg, color: cfg.color, fontSize: 10, fontWeight: 700 }}>{cfg.short}</span>
                                {act.description && <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", opacity: 0.8 }}>{act.description.slice(0, 50)}{act.description.length > 50 ? "…" : ""}</span>}
                              </div>
                            </div>
                            <div style={{ textAlign: "right", flexShrink: 0 }}>
                              <div style={{ fontSize: 11, color: "var(--text-muted)" }}>{format(new Date(act.created_at), timestampFmt)}</div>
                              <div style={{ display: "inline-flex", alignItems: "center", gap: 3, marginTop: 3, padding: "2px 7px", borderRadius: 99, fontSize: 10, fontWeight: 700, background: done ? "rgba(16,185,129,0.1)" : isMeet ? "rgba(239,68,68,0.08)" : "rgba(245,158,11,0.08)", color: done ? "#10B981" : isMeet && !done ? "#EF4444" : "#F59E0B" }}>
                                {done ? <CheckCircle2 size={8} /> : <Clock size={8} />}
                                {done ? "Done" : act.status === "in_progress" ? "In Progress" : isMeet ? "Not Completed" : "Pending"}
                              </div>
                            </div>
                          </motion.div>
                        );
