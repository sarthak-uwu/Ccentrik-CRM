...
import {
  ...
  CheckSquare, Square, UserCheck, ChevronLeft, ChevronRight,
} from "lucide-react";
...
const SOURCE_LABELS = {
  ...
  social_media: "Social Media", ads: "Ads", walk_in: "Walk-In", google: "Google", other: "Others",
};
...
const SOURCE_CONFIG = {
  ...
  google:         { icon: GoogleLogo,      color: "#4285F4" },
...
function PipelineFormModal({ entry, onClose, onSave, teamMembers = [], canAssign = false }) {
  ...
  const addPerson = () => {
    const id = nanoid();
    setPersons((prev) => [...prev, { id, name: "", designation: "", email: "", phone: "", linkedin: "", is_primary: false }]);
  };
                ...
                <div key={p.id} style={{ padding: "14px 16px", borderRadius: 12, background: "var(--surface-2)", border: `1.5px solid ${pocId === p.id ? "rgba(139,92,246,0.4)" : "var(--border)"}`, marginBottom: 10, position: "relative" }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
                    ...
                    <div style={{ gridColumn: "1 / -1", position: "relative" }}>
                      <label className="crm-label">LinkedIn Profile URL</label>
                      <input className="crm-input" value={p.linkedin || ""} onChange={(e) => updatePerson(p.id, "linkedin", e.target.value)} placeholder="https://linkedin.com/in/username" style={{ paddingRight: p.linkedin ? 36 : undefined }} />
                      {p.linkedin && (
                        <button type="button" onClick={() => window.open(p.linkedin.startsWith("http") ? p.linkedin : `https://${p.linkedin}`, "_blank")} title="Open LinkedIn profile"
                          style={{ position: "absolute", right: 10, top: "calc(50% + 9px)", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "#0A66C2", padding: 2, display: "flex", alignItems: "center" }}>
...
// ─── Main Pipeline Page ────────────────────────────────────────────────────────
// ── CSV helpers ───────────────────────────────────────────────────────────────
const CSV_TEMPLATE_HEADERS = ["Company Name","Contact Name","Designation","Pipeline Stage","Source","Industry","Country","State","City","Email","Phone","Website","Notes","LinkedIn Profile URL"];
const CSV_TEMPLATE_ROW     = ["Acme Corp","Jane Doe","Director","new_prospect","linkedin","Technology","India","Maharashtra","Mumbai","jane@acme.com","+91 9876543210","https://acme.com","Interested in SAP","https://linkedin.com/in/janedoe"];
...
function validateRow(row) {
  ...
  if (row.linkedin_profile_url && !linkedinRe.test(row.linkedin_profile_url.trim())) errs.push("Invalid LinkedIn URL");
  if (row.source) {
    const raw = row.source.trim().toLowerCase().replace(/\s+/g, "_");
    const byLabel = LEAD_SOURCES.find((s) => s.label.toLowerCase().replace(/\s+/g, "_") === raw);
    if (!VALID_SOURCES.has(raw) && !byLabel) errs.push(`Unknown source "${row.source}" — will be cleared`);
  }
  return errs;
}
...
function InlineSourceSelect({ value, sources, onSave, disabled, sourceConfig }) {
  const [open, setOpen] = useState(false);
  const ref = useRef();
  useEffect(() => {
    if (!open) return;
  ...
  const cur     = sources.find((s) => (s.key || s.value) === value);
  const curCfg  = sourceConfig?.[value];
  const curColor = curCfg?.color || "#6366F1";
    ...
    <div ref={ref} style={{ position: "relative", display: "inline-block" }} onClick={(e) => e.stopPropagation()}>
      <button
        ...
        style={{
          ...
          background: cur ? `${curColor}12` : "var(--surface-2)",
          color: cur ? curColor : "var(--text-muted)",
          fontWeight: 600, fontSize: 11.5,
          border: `1px solid ${curColor}28`,
      ...
      >
        {cur ? (
          <>
            {value === "google" ? <GoogleLogo size={13} /> : (CurIcon && <CurIcon size={13} strokeWidth={1.75} style={{ flexShrink: 0 }} />)}
            {cur.label}
          </>
        ) : <span style={{ color: "var(--text-muted)", fontWeight: 400, fontSize: 12 }}>—</span>}
        {!disabled && <ChevronDown size={9} style={{ opacity: 0.55 }} />}
      </button>
      {open && (
        <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, zIndex: 400, background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 10, boxShadow: "0 8px 28px rgba(0,0,0,0.14)", padding: "4px 0", minWidth: 185, maxHeight: 240, overflowY: "auto" }} className="custom-scroll">
          <div style={{ padding: "3px 12px 5px", fontSize: 9.5, fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.07em" }}>Lead Source</div>
          {sources.map((s) => {
            const k = s.key || s.value; const isSel = k === value;
            const sCfg = sourceConfig?.[k];
            const SIcon = sCfg?.icon;
            const sColor = sCfg?.color || "#6366F1";
            return (
              <button key={k} onClick={() => { onSave(k); setOpen(false); }}
                style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", padding: "7px 12px", border: "none", background: isSel ? `${sColor}12` : "transparent", cursor: "pointer", textAlign: "left", fontSize: 12.5, color: isSel ? sColor : "var(--text)", fontWeight: isSel ? 700 : 400, fontFamily: "inherit" }}
                onMouseEnter={(e) => e.currentTarget.style.background = `${sColor}12`}
                onMouseLeave={(e) => e.currentTarget.style.background = isSel ? `${sColor}12` : "transparent"}
              >
                {k === "google" ? <GoogleLogo size={14} /> : (SIcon && <SIcon size={14} strokeWidth={1.75} style={{ flexShrink: 0, color: sColor }} />)}
                <span style={{ flex: 1 }}>{s.label}</span>
                {isSel && <span style={{ fontSize: 10, color: sColor }}>✓</span>}
              </button>
            );
          })}
        </div>
...
// ─── Inline Stage Select ──────────────────────────────────────────────────────
function InlineStageSelect({ value, stages, onSave, disabled }) {
  const [open, setOpen] = useState(false);
  const ref = useRef();
  useEffect(() => {
    if (!open) return;
  ...
  const cur = stages.find((s) => s.key === value);
  return (
    <div ref={ref} style={{ position: "relative", display: "inline-block" }} onClick={(e) => e.stopPropagation()}>
      <button
        onClick={() => !disabled && setOpen((v) => !v)}
        style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "3px 7px 3px 6px", borderRadius: 6, background: cur?.bg || "var(--surface-2)", color: cur?.color || "var(--text-muted)", fontWeight: 700, fontSize: 11, border: `1.5px solid ${(cur?.color || "#888")}28`, cursor: disabled ? "default" : "pointer", fontFamily: "inherit" }}
      >
        <span style={{ width: 6, height: 6, borderRadius: "50%", background: cur?.color || "#888", flexShrink: 0 }} />
        {cur?.label || value || "—"}
        {!disabled && <ChevronDown size={9} style={{ opacity: 0.55 }} />}
      </button>
      {open && (
        <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, zIndex: 400, background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 10, boxShadow: "0 8px 28px rgba(0,0,0,0.14)", padding: "4px 0", minWidth: 185 }}>
          {stages.map((s) => {
            const isSel = s.key === value;
            return (
              <button key={s.key} onClick={() => { onSave(s.key); setOpen(false); }}
                style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", padding: "7px 12px", border: "none", background: isSel ? (s.bg || "var(--surface-2)") : "transparent", cursor: "pointer", textAlign: "left", fontFamily: "inherit" }}
                onMouseEnter={(e) => e.currentTarget.style.background = s.bg || "var(--surface-2)"}
                onMouseLeave={(e) => e.currentTarget.style.background = isSel ? (s.bg || "var(--surface-2)") : "transparent"}
              >
                <span style={{ width: 8, height: 8, borderRadius: "50%", background: s.color, flexShrink: 0 }} />
                <span style={{ fontSize: 12.5, fontWeight: isSel ? 700 : 400, color: isSel ? s.color : "var(--text)" }}>{s.label}</span>
                {isSel && <span style={{ marginLeft: "auto", fontSize: 10, color: s.color }}>✓</span>}
              </button>
            );
          })}
        </div>
...
export default function Pipeline() {
  ...
  const canDelete = isSalesHead; // Bulk-delete: admin only; per-row delete also allowed for own records (see canDeleteThis)
  ...
  const pendingActivityRef = useRef(searchParams.get("activity"));
  const [search,          setSearch]          = useState("");
  ...
  const { data: pipelineData, isPending: pipelineIsPending, isError, error: pipelineError, refetch: refetchPipeline } = useQuery({
    queryKey: ["pipeline", search, profile?.id],
    queryFn:  () => leadsService.getPipelineEntries({ search, profileId: profile?.id, role: profile?.role }),
    retry: 1,
    enabled: !!profile?.id,
  });
  const isLoading = pipelineIsPending;
  ...
  // Count of leads created per pipeline entry — scoped to own records for field users
  const { data: leadsCountMap = {} } = useQuery({
    queryKey: ["pipeline-leads-counts", isFieldUser ? profile?.id : "all"],
    queryFn: async () => {
      let q = supabase.from("leads").select("pipeline_id")
        .not("pipeline_id", "is", null)
        .neq("stage", "pipeline");
      if (isFieldUser && profile?.id) q = q.eq("assigned_to", profile.id);
      const { data } = await q;
      const map = {};
  ...
  // Auto-open detail panel when navigated via ?selected=<id>&activity=<id> (from dashboard alerts)
  useEffect(() => {
    const targetId = pendingSelectRef.current;
    if (!targetId || !allEntries.length) return;
    const match = allEntries.find((e) => e.id === targetId);
    if (match) {
      setSelectedEntry(match);
      if (pendingActivityRef.current) {
        setHighlightActivityId(pendingActivityRef.current);
        pendingActivityRef.current = null;
      }
      pendingSelectRef.current = null;
      setSearchParams({}, { replace: true });
    }
  }, [allEntries, setSearchParams]);
  ...
  const filteredEntries = useMemo(() => {
    ...
    rows.sort((a, b) => {
      let av, bv;
      if (sortBy === "lead_code") {
        // Sort numerically by the numeric portion of the lead code (e.g. "001" → 1)
        const num = (v) => parseInt((v || "").replace(/\D/g, "") || "0", 10);
        av = num(a.lead_code); bv = num(b.lead_code);
      } else if (sortBy === "company_name") { av = (a.company_name || "").toLowerCase(); bv = (b.company_name || "").toLowerCase(); }
  ...
  // ── Pagination ────────────────────────────────────────────────────────────
  const totalPipelinePages = Math.max(1, Math.ceil(filteredEntries.length / PAGE_SIZE));
  const safePipelinePage   = Math.min(pipelinePage, totalPipelinePages);
  const paginatedEntries   = filteredEntries.slice((safePipelinePage - 1) * PAGE_SIZE, safePipelinePage * PAGE_SIZE);
  useEffect(() => { setPipelinePage(1); }, [filteredEntries]);
  ...
  const createMutation = useMutation({
    mutationFn: (data) => {
      // Field users (employee / inside_sales) are always assigned to themselves — no override allowed
      const assignedTo = isFieldUser ? profile?.id : (data.assigned_to || profile?.id);
      return leadsService.create({ ...data, created_by: profile?.id, is_locked: false, assigned_to: assignedTo });
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["pipeline"] }); toast.success("Prospect added!"); setShowForm(false); },
  ...
  const bulkImportMutation = useMutation({
    mutationFn: async ({ rows, assignedTo }) => {
      ...
      const records = rows.map((r) => ({
        ...
        pipeline_stage: (() => {
          const ps = (r.pipeline_stage || "").toLowerCase().trim().replace(/\s+/g, "_");
          return PIPELINE_STAGES.some((s) => s.key === ps) ? ps : "new_prospect";
        })(),
        source: (() => {
          const raw = (r.source || "").trim().toLowerCase().replace(/\s+/g, "_");
          // Accept exact key match OR label match (e.g. "Cold Call" → "cold_call")
          const byKey   = LEAD_SOURCES.find((s) => s.key === raw);
          const byLabel = LEAD_SOURCES.find((s) => s.label.toLowerCase().replace(/\s+/g, "_") === raw);
          return (byKey || byLabel)?.key || null;
        })(),
  ...
  const inlineUpdateMutation = useMutation({
    mutationFn: async ({ id, field, value }) => {
      if (field === "pipeline_stage") return leadsService.updatePipelineStage(id, value);
      return leadsService.update(id, { [field]: value });
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["pipeline"] }); toast.success("Updated"); },
    onError: (e) => toast.error(e.message),
  });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["pipeline"] }); toast.success("Prospect updated"); setEditEntry(null); },
    onError:   (e) => toast.error(e.message),
  });

  const deleteMutation = useMutation({
    mutationFn: leadsService.delete,
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["pipeline"] }); toast.success("Prospect removed"); },
    onError:   (e) => toast.error(e.message),
  });
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "0 24px 24px" }}>
        ...
        {isLoading ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "80px 0", gap: 14 }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 700, color: "var(--text)" }}>Failed to load pipeline data</div>
            <div style={{ fontSize: 13, color: "var(--text-muted)", maxWidth: 380, textAlign: "center", lineHeight: 1.6 }}>
              {pipelineError?.message || "Could not connect to the server. Check your connection and try again."}
            </div>
            <button className="btn-primary" onClick={() => refetchPipeline()} style={{ marginTop: 4, display: "flex", alignItems: "center", gap: 6 }}>
              Retry
            </button>
          </div>
                    ...
                    {paginatedEntries.map((e, i) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td>
                            {extra.country ? (
                              <span style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 12.5, color: "var(--text-2)", fontWeight: 500 }}>
                                <FlagImg code={extra.country} size={20} />
                          ...
                          <td onClick={(ev) => ev.stopPropagation()}>
                            <InlineSourceSelect
                              value={e.source || extra.source || null}
                              sources={LEAD_SOURCES}
                              sourceConfig={SOURCE_CONFIG}
                              disabled={e.is_locked && !isSalesHead}
                              onSave={(v) => inlineUpdateMutation.mutate({ id: e.id, field: "source", value: v })}
                            />
                          </td>
                          )}
                          ...
                          <td>
                            {(() => {
                              const liUrl = extra.company_linkedin
                                || extra.linkedin_url
                                || contacts.find((c) => c.type === "linkedin")?.value;
                              if (!liUrl) return <span style={{ color: "var(--text-muted)", fontSize: 11 }}>—</span>;
                              const href = liUrl.startsWith("http") ? liUrl : `https://${liUrl}`;
                              const display = liUrl.replace(/^https?:\/\/(www\.)?/i, "").replace(/\/$/, "");
                              return (
                                <a href={href} target="_blank" rel="noopener noreferrer" title={liUrl}
                                  onClick={(ev) => ev.stopPropagation()}
                                  style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 11.5, color: "#0A66C2", textDecoration: "none", maxWidth: 130 }}>
                                  <LinkedinIcon size={12} style={{ flexShrink: 0 }} />
                                  <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                                    {display.length > 20 ? display.slice(0, 20) + "…" : display}
                                  </span>
                                  <ExternalLink size={9} strokeWidth={2} style={{ flexShrink: 0 }} />
                                </a>
                              );
                            })()}
                          </td>
                          ...
                          <td onClick={(ev) => ev.stopPropagation()}>
                            <InlineStageSelect
                              ...
                              disabled={e.is_locked && !isSalesHead}
                              onSave={(v) => inlineUpdateMutation.mutate({ id: e.id, field: "pipeline_stage", value: v })}
                            />
                          </td>
                          )}
                          ...
                          <td style={{ textAlign: "right" }} onClick={(ev) => ev.stopPropagation()}>
                            <div style={{ display: "flex", alignItems: "center", gap: 4, justifyContent: "flex-end" }}>
                              {(() => {
                                ...
                                const canDeleteThis = isSalesHead;
      ...
      <AnimatePresence>
        {selectedEntry && (
          <PipelineDetailPanel
            ...
            onClose={() => { setSelectedEntry(null); setHighlightActivityId(null); }}
            onEdit={(e) => { setEditEntry(e); setSelectedEntry(null); }}
            onConvert={handleConvertClick}
            highlightActivityId={highlightActivityId}
            onHighlightConsumed={() => setHighlightActivityId(null)}
          />
        )}
      </AnimatePresence>
    </div>
