...
const METRICS = [
  { key: "qualified_leads", label: "Qualified Leads",  icon: TrendingUp, color: "#8B5CF6", unit: "leads"     },
  { key: "meetings",        label: "Meetings",          icon: Calendar,   color: "#3B82F6", unit: "meetings"  },
  { key: "activities",      label: "Activities",        icon: Activity,   color: "#F59E0B", unit: "activities"},
  { key: "revenue",         label: "Revenue (Won)",     icon: Briefcase,  color: "#10B981", unit: "currency"  },
  { key: "deals_won",       label: "Deals Won",         icon: Trophy,     color: "#06B6D4", unit: "deals"     },
  { key: "deals_proposal",  label: "Deals in Proposal", icon: Target,     color: "#F97316", unit: "deals"     },
  { key: "custom",          label: "Custom Metric",     icon: Edit2,      color: "#64748B", unit: "items"     },
];
...
async function fetchAllProgress() {
  const [leadsRes, activitiesRes, dealsRes, meetingsRes] = await Promise.all([
    ...
    supabase.from("meetings").select("created_by, status, start_time, created_at, lead_id, deal_id"),
  ]);
  return {
    leads:      leadsRes.data      || [],
...
function computeProgress(target, progressData) {
  ...
  if (target.metric === "meetings") {
    // Count any non-cancelled meeting belonging to the assigned user within the period
        ...
        <form onSubmit={handleSubmit(onSubmit)} style={{ padding: 24, display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div>
            ...
            >
              ...
              <option value="deals_proposal">Deals in Proposal</option>
